...
### innodb_page_size 是什么?

> **innodb_page_size 是 InnoDB 的 “数据页大小”**，单位字节，默认 **16384（16KB）**MySQL。
>
> - 它是 InnoDB **读写磁盘的最小单位**（决定一次 I/O 读写多大）：哪怕只查一行，也会把**整页**（16KB）读到内存缓冲池。
> - 所有数据、索引、Undo Log 都按 “页” 组织。
> - **只能在 MySQL 初始化时设置，之后不能改**MySQL
> - 只支持：**4K / 8K / 16K / 32K / 64K**（4096、8192、16384、32768、65536）
...
### innodb_page_size 对 B+ 树的影响

> **InnoDB 的 1 个 B+ 树节点 = 1 个 innodb_page**
>
> - 页越大 → 单个节点能存的**索引条目越多**
> - 页越小 → 单个节点能存的**索引条目越少**
...
>
> - **默认页填充率**：**15/16（即保留 1/16 空闲）**
>
> 所以**真正能存数据的空间**是：
>
> ```
> 16384 × 15/16 ≈ 15360 字节
> ```
>
> ------
>
...
> 8KB 可用空间 ≈ 7680 字节
>
> - BIGINT：7680 ÷14 ≈ **548 条 / 页**
> - 3 层只能存：548×548×548 ≈ **1.6 亿**
> - 要存 13 亿，必须 **4 层**
>
> 结论：
>
